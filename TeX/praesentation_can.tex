...
\begin{frame}{Geschichte} 
	\begin{itemize}
		...
		\item Ziel war die Reduzierung der Kabelbaumlänge in Fahrzeugen \\ \pause
...
\begin{frame} 		
	\frametitle{Subtraktion mit Hilfe des Zweierkomplements}
		...
		\hspace*{5mm} $ 100 \Rightarrow 4$ ; da das höchstwertigste Bit 1 ist: \\ \hspace*{5mm} Endergebnis $= -4$\\ \pause
		9. Schritt: Ergebnis:\\ 
...
\begin{frame}{Multiplikation von Dualzahlen}
	\begin{itemize}
		\item Es werden Produkte mit den einzelnen Stellen des Multiplikators gebildet,sie werden anschließend Stellenrichtig addiert. \\ \pause 
		\item Die Stellen des Multiplikators können nur Zahlenwerte zwischen Null und Eins annehmen \\ \pause
		\item $\rightarrow$ Dies kann mit einer einfachen UND-Verknüpfung gelöst werden.
	\end{itemize}
\end{frame}
...
\begin{frame}{Halbaddierer}
	\begin{columns}
		\begin{column}{0.35\linewidth}
			Ein Halbaddierer besitzt zwei Ein-, und zwei Ausgänge. An die Eingänge \emph{x} und \emph{y} werden jeweils die 
			Ziffern angelegt die man addieren möchte. An dem ersten Ausgang liegt die Summe $s$ der Addition an,
			am zweiten Ausgang der Übertrag $c$.
		\end{column}
	...
	\begin{columns}
		\begin{column}{0.52\linewidth}
			\begin{itemize}
				...
				\item Der Volladdierer hat einen zusätzlichen Eingang ($c_{in}$), man kann den Übertrag aus einer vorhergegangenen Addition
					mit einbeziehen. \\ \pause
				\item Man kann mehrere Volladdierer kaskadieren um größere Zahlen miteinander zu addieren.
			\end{itemize}
		\end{column}
...
\begin{frame}{Subtrahierer}
		...
		$0$, und für eine Subtraktion eine $1$ an. Durch diesen Steuereingang wird das Zweierkomplement gebildet.
%		\begin{figure}
...
\begin{frame}{Multiplizierer}
		...
		werden kann.\\ \pause
		\item Die höchstwertigste Ziffer des Multiplikators wird abgegriffen, und mit dem Multiplikant multipliziert. \\ \pause
		\item Um die Produkte Stellenrichtig zu addieren werden die Zwischenergebnisse ebenfalls verschoben. \\ \pause
		\item Das Produkt erhält man, nach acht Takten Rechenzeit.
		\end{itemize}
\end{frame}
...
\begin{frame}{BCD-Eingabe}
		\begin{itemize}
		\item Die Eingabe erfolgt über ein ASCII-Feld. \\ \pause
		\item Bei einer Eingabe von einer Ziffer im Bereich 0 bis 9, liegt die Ziffer BCD codiert an den Ausgänge $D_{0}$ bis $D_{3}$ an\\ \pause
		\item  Nach jedem Tastendruck wird die Ziffer in ein Register übernommen, man kann an ihren Ausgängen die Zahl von 0 bis
			 99 abgreifen. 
		\end{itemize}
\end{frame}
...
\begin{frame}{Umwandlung}
		\begin{itemize}
		\item Für die Wandlung werden zwei Zähler und ein Wandler benötigt.\\ \pause
		\item Der erste Zähler ist ein BCD-Zähler, der zweite ein Binär-Zähler.\\ \pause
		\item  Beide Zähler haben den selben Clock.\\ \pause
		\item  Der Vergleicher vergleicht das eingegebene Signal mit dem BCD-Zähler. \\ \pause
		\item  Sind beide Signale identisch wird der Clock unterbrochen.\\ \pause
		\item  Man kann das Binär-Ergebnis am Ausgang des Binär-Zählers ablesen.
		\end{itemize}
\end{frame}
...
\begin{frame}{7-Segment-Anzeige}
		\begin{itemize}
		\item Das Endergebnis wird Ziffer für Ziffer auf ein 7-Segment-Decoder gegeben.\\ \pause
		\item Die Ausgänge des 7-Segment-Decoders werden auf die Eingänge einer 7-Segment-Anzeige gegeben.
		\end{itemize}
\end{frame}
